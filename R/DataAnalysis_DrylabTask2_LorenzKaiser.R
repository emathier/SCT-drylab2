...
new_ident <- setNames(celltype,
                      levels(seurat))
top_peaks_ct$celltype <- celltype[as.integer(top_peaks_ct$group)+1]

seurat <- RenameIdents(seurat, new_ident)

p1 <- UMAPPlot(seurat, label=T, label.size = 5, label.box = TRUE) & NoAxes()
p1
...
#-------------- TF Binding mofif enrichment analysis --------------
...
# --> this step used 44GB of RAM, after which my laptop chrashed
